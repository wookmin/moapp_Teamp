                      ...
                      children: [
                        ...
                        Text.rich(TextSpan(
                          ...
                          children: [
                            ...
                            TextSpan(text: '${summary.urgentCount}개',
...
class _StatusInfo {
  ...
  factory _StatusInfo.fromScore(int score) {
    if (score >= 80) return const _StatusInfo(message: '신선해요', color: Color(0xFF1B6B47));
    if (score >= 50) return const _StatusInfo(message: '조심하세요', color: Color(0xFFD98A00));
      ...
      children: [
        Row(
          ...
          children: [
            ...
            TextButton(
              onPressed: () => Navigator.of(context).pushNamed('/expiry-management'),
        ...
        if (foods.isEmpty)
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, i) => _UrgentFoodCard(food: foods[i]))),
      ],
    );
  }
}
...
class _UrgentFoodCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
    final highlight = food.isUrgent ? const Color(0xFFFCEAEA) : colorScheme.surface;
    final tagColor = food.isUrgent ? const Color(0xFFC0392B) : colorScheme.primary;

    return SizedBox(
      width: 210,
      child: Card(
        elevation: 0, clipBehavior: Clip.antiAlias, color: highlight,
        child: Padding(
          padding: const EdgeInsets.all(14),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Icon(Icons.restaurant_rounded, color: colorScheme.onSurfaceVariant),
              const Spacer(),
              Row(children: [
                Expanded(child: Text(food.name, maxLines: 1, overflow: TextOverflow.ellipsis,
                  style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700))),
                _Tag(label: food.statusLabel, color: tagColor),
              ]),
              const SizedBox(height: 8),
              Text(food.expiryLabel, maxLines: 1, overflow: TextOverflow.ellipsis,
                style: theme.textTheme.bodySmall?.copyWith(color: colorScheme.onSurfaceVariant)),
            ],
          ),
        ),
...
class _AiRecipeCard extends StatelessWidget {
  const _AiRecipeCard({required this.recipe, required this.onRefresh, this.error});
  final Recipe? recipe;
  final String? error;
  ...
  Widget build(BuildContext context) {
    if (recipe == null) {
      ...
      return EmptyStateView(
        ...
        action: TextButton.icon(onPressed: onRefresh,
          icon: const Icon(Icons.refresh_rounded, size: 18), label: const Text('다시 시도')),
      );
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFF1B6B47), borderRadius: BorderRadius.circular(20)),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [
            ...
            Text('AI 추천 레시피', style: theme.textTheme.labelMedium?.copyWith(
          ...
          Text(recipe!.title, style: theme.textTheme.titleLarge?.copyWith(
            color: Colors.white, fontWeight: FontWeight.w800)),
          const SizedBox(height: 10),
          Text(recipe!.summary, style: theme.textTheme.bodyMedium?.copyWith(
            color: Colors.white.withValues(alpha: 0.85), height: 1.5)),
          if (recipe!.ingredients.isNotEmpty) ...[
            const SizedBox(height: 16),
            Text('필요한 재료', style: theme.textTheme.labelLarge?.copyWith(
            ...
            Wrap(spacing: 8, runSpacing: 6,
              children: recipe!.ingredients.map((ing) => Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.white.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(999)),
                child: Text(ing, style: theme.textTheme.labelMedium?.copyWith(color: Colors.white)),
              )).toList()),
          ],
          if (recipe!.steps.isNotEmpty) ...[
            const SizedBox(height: 16),
            Text('조리 순서', style: theme.textTheme.labelLarge?.copyWith(
              color: Colors.white, fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
                ...
                Container(width: 22, height: 22, margin: const EdgeInsets.only(right: 8, top: 1),
                  ...
                  child: Center(child: Text('${e.key + 1}',
                    style: theme.textTheme.labelSmall?.copyWith(
                      color: Colors.white, fontWeight: FontWeight.w700)))),
                Expanded(child: Text(e.value, style: theme.textTheme.bodySmall?.copyWith(
